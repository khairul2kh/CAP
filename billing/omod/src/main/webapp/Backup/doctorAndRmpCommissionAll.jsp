...
<form class="form-rep-view" method="get" action="compayview.htm" id="commissionCalForm" onsubmit="return validate()" >
    ...
    <div class="box1">
        ...
        <c:set var="billIdForLoopingStatus" value=""/>
        <c:set var="doctorIdForLoopingStatus" value=""/>
        <c:set var="commaSeparatedServiceNames" value=""/>
        <c:set var="subTotalServicePrice" value="${0}"/>
        <c:set var="subTotalCommissionAmount" value="${0}"/>
        <c:set var="subTotalRefferalAmount" value="${0}"/>

        <c:set var="subTotalLessAmount" value="${0}"/>
        <c:set var="subTotalCommissionInPercentage" value="${0}"/>
        <c:set var="firstBillId" value="${0}"/>
        ...
        <c:if test="${docch eq '1'}">
            <center><b style="color:black; font-size:18px;  "> Referral Doctor List </b> </center>
                ...
                <c:forEach items="${allDoctorAndRmpCommission}" var="d" varStatus="loop" >
                    ...
                    <c:set var="doctorCommission"> <fmt:parseNumber  type="number" value="${d.commission}" /> </c:set>
                    ...
                    <c:set var="actualRefferel" value="${ (ssss * doctorCommission)/100 }"/>
                    ...
                    <c:if test="${d.diaPatientServiceBill.billingStatus eq 'PAID'}">   
                        ...
                        <c:choose>
                            ...
                            <c:when test="${ totper > 20}">
                                <c:set var="totalreferal" value="${ actualRefferel-ll}"/>

                            </c:when>						

                            <c:when test="${ totper <= 20 }">
                                <c:set var="totalreferal" value="${ (actualRefferel-(ll/2))}"/>
                    ...
                    <c:if test="${empty billIdForLoopingStatus}">

                        <c:set var="billIdForLoopingStatus" value="${d.diaPatientServiceBill.billId}"/>
                        <c:set var="firstBillId" value="${d.diaPatientServiceBill.billId}"/>

                    </c:if>



                    <c:if test="${empty doctorIdForLoopingStatus}">


                        <c:set var="doctorIdForLoopingStatus" value="${d.doctorDetails.id}"/>                                        
                    </c:if>



                    <c:choose>
                        <c:when  test="${billIdForLoopingStatus != d.diaPatientServiceBill.billId}">

                            <c:if test="${doctorIdForLoopingStatus == d.doctorDetails.id}">
                                <table style=" font-size:12px;  border-collapse: collapse; border :1pt solid #999; width: 100%;  font-family:arial;"  >  						
                                    ...
                                    <tr> 
                                        ...
                                        <td  class="a left"> ${billIdForLoopingStatus}    ${d.doctorDetails.id} </td>  <!-- bill id -->
                                        <td  class="a left"> ${commaSeparatedServiceNames}   </td> <!-- investigation name -->
                                        <td  align="right" class="a right"> ${subTotalServicePrice}   </td>  <!-- rate -->
                                        <td  align="right" class="a right">   ${subTotalCommissionAmount}    </td>  <!-- referrel amount -->
                                        <td  align="right" class="a right"  >  ${subTotalLessAmount}   </td> <!-- discount -->
                                        <td  align="center" class="a right"  >                                 					
                                            <fmt:formatNumber type="number" minFractionDigits="2"  value="${(subTotalLessAmount/subTotalServicePrice)*100}" />
                                        ...
                                        <td  align="right" class="a right"> <c:if test="${d.diaPatientServiceBill.billingStatus eq 'DUE'}">  DUE  </c:if>
                                            ...
                                            <c:if test="${d.diaPatientServiceBill.billingStatus eq 'PAID'}">
                                                <fmt:formatNumber type="number" maxFractionDigits="3" value="${subTotalRefferalAmount}" /> 												
                                            </c:if> 
                                        </td>     <!-- total referrel -->
                                    </tr>
                                    ...
                                    <b> Referral  :  ${d.doctorDetails.id}  &emsp;&emsp;&emsp; ${d.doctorDetails.doctorName}, ${d.doctorDetails.dergee} </b> <br> 
                                    <c:if test="${not empty d.doctorDetails.address}" > <b> Address : </b> ${d.doctorDetails.address} </c:if> </br> &nbsp;
                                        ss
                                ...
                                <c:if test="${doctorIdForLoopingStatus != d.doctorDetails.id}">	
                                    ...
                                    <c:set var="netTotalReferal" value="${0}" />
                                    <c:set var="rate" value="${0}" />
                                    <c:set var="refAmount" value="${0}" />
                                    <c:set var="discount" value="${0}" />
                            ...
                            <c:otherwise>
                                <c:choose>
                                    <c:when  test="${loop.last}">
                                        ...
                                        <c:set var="currentServiceName" value="${d.serviceName}"/>
                                        <c:set var="comma" value=","/>
                                        <c:set var="commaSeparatedServiceNames" value="${commaSeparatedServiceNames} ${currentServiceName} ${comma} "/>									
                                        <c:set var="subTotalServicePrice" value="${subTotalServicePrice+d.servicePrice}"/>
                                        <c:set var="subTotalCommissionAmount" value="${subTotalCommissionAmount+actualRefferel}"/>
                                        <c:set var="subTotalLessAmount" value="${subTotalLessAmount+ d.lessAmount}"/>
                                        <c:set var="subTotalRefferalAmount" value="${subTotalRefferalAmount+totalreferal}"/>       


                                        <c:set var="rate" value="${rate + d.servicePrice}"/>
                                        <c:set var="refAmount" value="${refAmount + actualRefferel}"/>
                                        <c:set var="discount" value="${discount + d.lessAmount}"/>
                                        <c:set var="netTotalReferal" value="${netTotalReferal + totalreferal}"/>

                                        <tr> 
                                            <td  class="a left"><openmrs:formatDate date="${d.createdDate}" /> </td> <!-- date -->
                                            <td  class="a left"> ${billIdForLoopingStatus}   </td>  <!-- bill id -->
                                            <td  class="a left"> ${commaSeparatedServiceNames}   </td> <!-- investigation name -->
                                            <td  align="right" class="a right"> ${subTotalServicePrice}   </td>  <!-- rate -->
                                            <td  align="right" class="a right">   ${subTotalCommissionAmount}    </td>  <!-- referrel amount -->
                                            <td  align="right" class="a right"  >  ${subTotalLessAmount}   </td> <!-- discount -->
                        ...
                        <table style=" font-size:12px;  border-collapse: collapse; border :1pt solid #999; width: 100%;  font-family:arial;"  >  						
                            ...
                            <c:forEach items="${listDia}" var="d" >
                                <c:if test="${q.id eq d.refRmp}" >
                                    ...
                                    <c:set var="ll"><fmt:parseNumber  type="number"    value="${d.lessAmount}" /> </c:set>
                                    <c:set var="ssss"> <fmt:parseNumber  type="number" value="${d.servicePrice}" /> </c:set>
                                    <c:set var="actualRefferel"> <fmt:parseNumber  type="number" value="${d.comAmount}" /> </c:set>

                                    <c:set var="totper" value="${(ll/ssss)*100}" />
                                    <c:set var="oneper" value="${(ssss*1)/100}" />

                                    <c:set var="doctorCommission" value="${(actualRefferel/ssss)*100} " /> 
                                    ...
                                    <c:if test="${d.diaPatientServiceBill.billingStatus eq 'PAID'}">    
                                        ...
                                        <c:if test="${ totper > '20.01' && totper < '30.01' }">
                                            <c:set var="totalreferal" value="${ actualRefferel -((totper - 20)* oneper)} " /> 
                                        </c:if>
